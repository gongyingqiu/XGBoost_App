...
#   in_res <- sym(in_response)
#   pred <- sym(pred_name)
#   average_prem <- sum(in_data[[pred_name]] * in_data[[in_exposure]])/sum(in_data[[in_exposure]])
#   
#   for(fac in in_fac_list) {
#     fac_sym <- sym(fac)
#     
      ...
      if ( n_distinct( rv$raw_data[[input$select_partition_factor]] ) == 3 | input$select_data_partition == '1' ){
        ...
        if(as.numeric(input$select_data_partition)==1){
          ...
          if (input$select_policy_keys == "NONE"){
            ...
            rv$key_data = subset_data[,c(input$select_policy_keys,input$select_partition_factor)] %>% partition_data_factor( input$select_partition_factor )
          }
